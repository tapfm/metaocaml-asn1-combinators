...
let (<|) (Oid (v1, v2, vs)) vn =
  if vn < 0
    then invalid_arg "OID.(<|): negative component: %d" vn;
  Oid(v1, v2, vs @ [vn]) (* I feel this could be an easy improvement *)

let (<||) (Oid (v1, v2, vs)) vs' = 
  let 
    test_args v = if v < 0
      then invalid_arg "OID.(<||): negative component: %d" v
  in
    List.iter test_args vs'; (* original has `vs` here -- possible bug?*)
    Oid (v1, v2, vs @ vs')
...
let of_string s =
  let rec go ic =
    if Scanf.Scanning.end_of_input ic
      then
        []
      else
        Scanf.bscanf ic ".%d%r" go (fun n ns -> n::ns)
  in
    try 
      Scanf.sscanf s "%d.%d%r" go of_nodes
    with
      End_of_file | Scanf.Scan_failure _ -> None
