...
module OID : Prim with type t = Asn_oid.t = struct
  ...
  let (<+>) = Asn_staged0_core.(<+>)

  (*
  let chain : bytes code -> int code -> int code  -> (int * int) code = fun bs i n ->
      let rec go : int64 code -> bytes code -> int code -> int code -> (int * int) code =
        fun acc bs i gs ->
          .<
            match .~ gs with 
            ...
            | n -> 
              ...
              | b ->
                ...
                if b < 0x80 then (Int64.to_int acc, .~i + 1) else .~(go .<acc>. bs .<.~i + 1>. .<n - 1>.)
          >.
    in
    .< 
  ...
  let of_bytes : bytes code -> t code = fun bs ->
    let rec components : bytes code -> int code -> int code -> int list code =
      fun bs i n ->
        .<
          if .~n = 0 then []
          else let (c, i') = .~(chain bs i n) in 
          c :: .~(components bs .<i'>. .<(.~n + .~i - i')>.)
        >.
    in
    .<
      match Bytes.length .~bs with 
